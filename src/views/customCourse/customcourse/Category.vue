...
import { customCourseCategory } from '@/api/customCourse/customCourse'
...
export default {
  ...
  created() {
    // this.topAdd();
    this.getList({ catParentId: 0, PageSize: 100 }) // 一级分类固定传0
  },
  methods: {
    // catParentId: 0 所有一级， pcatParentIdarentId: 传具体的就带边查子级
    // 初始化-查询左右
    async getList(data) {
      const { resObject: { list: resultLeft } } = await customCourseCategory.load(data)
      this.leftData = resultLeft
    },

    // 仅处理右边
    async getRightList(catParentId, categoryName = this.rightValue) {
      const onlyRight = await customCourseCategory.load({
    ...
    // 左搜
    leftSearch() {
      this.getList({ catParentId: 0, categoryName: this.leftValue, PageSize: 100 })
    },
    // 右搜
    rightSearch() {
    ...
    async onDelete(i, type) {
      ...
      this.$confirm('确定删除该数据？', '删除信息', {
        ...
        customCourseCategory.delete({ ids: ids }).then(() => {
    ...
    saveTag(formName) {
      ...
      this.$refs[formName].validate(valid => {
        ...
        customCourseCategory.add(params).then(() => {
          that.$message({
            message: '操作成功!',
            type: 'success'
          })
    ...
    editTag(formName) {
      this.$refs[formName].validate(valid => {
        ...
        customCourseCategory.update(params).then(() => {
          this.$message({
            message: '操作成功!',
            type: 'success'
          })
