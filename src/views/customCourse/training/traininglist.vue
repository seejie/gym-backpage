  ...
  <div >
          ...
          <el-row>
            <el-col :span="8">
              <el-form-item prop="title" label="搜索标题">
                <el-input v-model="searchData.trainingTitle" placeholder="请输入" ></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
      ...
      <el-card shadow="never" class="card">
        ...
        <div slot="header" class="clearfix">
          <el-button type="primary" @click="$router.push('training/Add/0/'+trainingType)">发布调研</el-button>
        ...
        <el-table :data="list"  style="width: 100%;" @selection-change="selectionchange">
          ...
          <el-table-column prop="trainingTitle" :label="'标题'" min-width="340px" >
            <template slot-scope="scope" >
              <el-row type="flex" align="middle" style="justify-content: left">
                ...
                <el-col :span="8">
                  <span style="margin-left:10px">{{scope.row.trainingTitle}}</span>
                </el-col>
              </el-row>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="230px">
            <template slot-scope="scope">
              ...
              <el-button type="text"><router-link :to="{name:'trainingAdd',params:{id:scope.row.id,type:trainingType}}">编辑</router-link></el-button>
              <el-button type="text"><el-link type="primary" @click="deleteone(scope.row.id)">删除</el-link></el-button>
              <el-button type="text"><el-link type="primary" @click="togglePublish(scope.row.id,scope.row.status==1?2:1)">{{scope.row.status==0||scope.row.status==null||scope.row.status==2?'上线':'下线'}}</el-link></el-button>
    ...
    <el-dialog class="showdata" :visible.sync="data.isshow" title="问卷数据" >
      <el-form class="el-row">
        <el-form-item>
          <el-row type="flex" >
            ...
            <el-col :span="7" style="display: flex">
              <el-button type="primary" @click="exportTestAndAnswerData">导出</el-button>
              <el-button v-if="trainingType==1" type="primary" @click="batchapproveTrainingTestData(1)">审核通过</el-button>
              <el-button v-if="trainingType==1" type="primary" @click="batchapproveTrainingTestData(2)">审核不通过</el-button>
      ...
      <el-table
        ref="datas" :data="data.list" tooltip-effect="dark" style="width: 100%" @selection-change="handleStudentSelectionChange">
        ...
        <el-table-column  v-for="(item,index) in data.header" :key="index" :label="item.name" show-overflow-tooltip>
          <template slot-scope="scope" >
            ...
            <el-switch v-else-if="item.key=='testStatus'" v-model="scope.row.testStatus==1" @change="approveTrainingTestData(scope.row.trainingRecordId,scope.row.testStatus==1?2:1)" active-color="#13ce66" inactive-color="#ff4949">
            </el-switch>
            <el-row v-else>
              {{scope.row[item.key]}}
            </el-row>
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      training.load(this.searchData).then(res => {
        if (res.resCode == 200) {
    ...
    showstatusdata(id) {
      ...
      this.data.list = []
      this.data.ids = []
      training.record({ trainingId: id, keyword: this.data.searchdata.key, pageSize: this.data.pageSize, pageNum: this.data.curPage }).then(res => {
        if (res.resCode == 200) {
    ...
    approveTrainingTestData(id, status) {
      var approveTrainingTestParam = { status: status, trainingRecordIdList: [] }
      approveTrainingTestParam.trainingRecordIdList.push(id)
      training.approveTrainingTestData(approveTrainingTestParam).then(res => {
        if (res.resCode == 200) {
    ...
    batchapproveTrainingTestData(status) {
