<template>
  <div >
    ...
    <el-card shadow="never" class="card">
      ...
      <el-table :data="list"  style="width: 100%;" @selection-change="selectionchange">
        ...
        <el-table-column label="操作" width="230px">
          <template slot-scope="scope">
            ...
            <el-button type="text"><el-link type="primary" v-if="scope.row.postSaleStatus==17&&scope.row.orderStatus==5" @click="getonedetail(scope.row.id,3)">退款</el-link></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
      ...
      <OrderDialog  ref='orderDialog' v-if="detaildata!=null" :productName="'课程名称'" @load="load" :type="type" :detailObj="detaildata"/>
    </el-card>
  </div>
</template>
...
import { order } from '@/api/customCourse/customCourse'
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      order.load(this.searchData).then(res => {
        if (res.resCode == 200) {
    ...
    batchPublish(status) {
      if (this.ids.length == 0) {
        ...
        var url = status == 1 ? order.finishCustomCourseOrder({ idList: this.ids }) : order.refundCustomCourseOrder({ idList: this.ids, status: 1 })
        url.then(res => {
          if (res.resCode == 200) {
            this.load()
