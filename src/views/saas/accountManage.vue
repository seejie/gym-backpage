<template>
  <div class="accountManage">
    ...
    <div class="from-wrap">
      <el-form :inline="true" ref="formInline" label-width="80px" label-position="left" :model="searchData">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item prop="title" label="所属单位">
    ...
    <el-card shadow="never" class="card">
      ...
      <el-dialog :close-on-click-modal="false" :title="form.id ? '编辑账号' : '新建账号'" :visible.sync="innerVisible">
        <el-form :model="form" :rules="rules" ref="form" :label-width="formLabelWidth">
          <!--begin 让浏览器自动填充到这个input-->
          <div class="replace-input"><input v-model="form.name" type="text" class="form-control" /></div>
          <div class="replace-input"><input v-model="form.password" type="password" class="form-control" /></div>
          ...
          <el-form-item label="昵称">
            <el-input type="text" v-model="form.name_true" autocomplete="new-text" :readonly="isReadonly"></el-input>
          </el-form-item>
          ...
          <el-form-item label="所属单位" prop="unitId">
            <el-select placeholder="请选择" v-model="form.unitId">
              <el-option v-for="item in units" :key="item.id" :label="item.unitName" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    batchTopicPublish(status, val) {
      ...
      let url = status == 0 ? `/sys/user/enable` : `/sys/user/disable`;
