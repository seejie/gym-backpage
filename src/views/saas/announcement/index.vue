    ...
    <el-card shadow="never" class="card">
      ...
      <el-table :data="issueData" style="width: 100%" @selection-change="selectionchange">
        ...
        <el-table-column prop="unname" label="用户评价" width="180px">
          <template slot-scope="scope">
            <el-rate
              v-model="scope.user_appraise"
              disabled
              show-score
              text-color="#ff9900"
              :score-template="scope.user_appraise">
            </el-rate>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="editHelp(scope.row.id)">编辑</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editHelp(val){
        console.log(val);
        this.$router.push({ name: `addAnnouncement`, params: {id: val}})
      },
      //下拉菜单点击事件
      dropdownMenu(data) {
        if (data.value == 1) {
        ...
        if (data.value == 5) {
          //删除
            this.$ajax.post('/setting/announceDel', {
            id: data.id
          })
            .then(res => {
              if (res.resCode == 200) {
        ...
        this.$ajax.get('/setting/announceList', {
          ...
          .then(res => {
            if (res.resCode == 200) {
              ...
              this.issueData = res.resObject.index;
            } else {
              this.message(res.message, 'error');
            }
          })
