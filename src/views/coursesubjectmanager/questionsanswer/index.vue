...
export default {
  ...
  methods: {
    ...
    goAddPage() {
      this.$router.push('/coursesubjectmanager/add-questionsanswer');
    },
    goEdit(obj) {
      this.$router.push(`/coursesubjectmanager/add-questionsanswer/${obj.id}`);
    },
    closeStudent() {
      this.showQues = false;
    },
    ...
    cancleTop(obj, num) {
      ...
      setQuestion({
        ...
        if (res.resCode == 200) {
          that.$message.success(num === 1 ? '置顶成功' : '取消置顶成功');
          that.onSearch();
        } else {
          that.$message.error(res.message);
        }
