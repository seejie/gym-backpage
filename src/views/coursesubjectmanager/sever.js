...
const service = {
  get: (url, params) => request({
    url: url,
    method: 'GET',
    params: assign(params || {}, { channelType: 9 }),
  }),
  post: (url, data) => request({
    url: url,
    method: 'POST',
    data: assign(data || {}, { channelType: 9 }),
  }),
}
...
export const questionColumnList = (param = {}) => service.get('/course/questionaire/listQuestionColumns', assign({}, param, { pageSize: 0, isActive: 1, type: 2 })); // 获取所有栏目列表
...
export const addColumn = (param) => service.post('/course/column/createColumn', param, { headers }); // 添加栏目分类
export const deleteColumn = (param) => service.post('/course/column/deleteColumn', param, { headers }); // 批量删除栏目分类
export const columnList = (param = {}) => service.get('/course/column/searchColumnPage', assign({}, param, { pageSize: 0 })); // 获取所有栏目列表
export const updateColumn = (id, param) => service.post(`/course/column/updateColumn/${id}`, param, { headers }); // 修改课程分类
export const activeColumn = (param) => service.post('/course/column/setActive', param, { headers }); // 修改课程分类
...
export const dealCommentList = (arr, obj) => {
  ...
  return list.map(v => {
    ...
    if (data) return assign(v, { name: data.nickName });
    return v;
  })
}
