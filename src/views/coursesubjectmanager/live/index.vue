  ...
  <div class="activeManage">
    ...
    <el-card class="box-card">
      <el-tabs v-model="tabActive" @tab-click="handleTabClick">
        <el-tab-pane label="直播管理" name="first">
          <el-form
            :inline="true"
            v-show="showSearch"
            ref="form"
          ...
          >
            ...
            <el-table-column prop="courseStatus" label="发布状态" width="120">
              <template slot-scope="scope">
                <span v-if="scope.row.courseStatus === 0" class="unline">待审核</span>
                <span v-if="scope.row.courseStatus === 1" class="online">已上线</span>
                <span v-if="scope.row.courseStatus === 2" class="downline">已下线</span>
              </template>
            </el-table-column>
            <el-table-column prop="columnSort" label="发布位置" width="120">
              <template slot-scope="scope">
                <span v-if="scope.row.columnSort === 0">默认</span>
                <span v-if="scope.row.columnSort === 1">置顶</span>
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="直播时段管理" name="second">
          <el-row class="add-btn">
            <span>时间段管理</span>
            <el-button type="primary" plain>新增时间段</el-button>
          </el-row>
        ...
        <el-tab-pane label="直播banner" name="banner">
          <banner></banner>
        </el-tab-pane>
      </el-tabs>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    goAddPage() {
      this.$router.push('/coursesubjectmanager/add-live');
    },
    goEdit(obj) {
      this.$router.push(`/coursesubjectmanager/add-live/${obj.id}`);
    },
    goComment(obj) {
