          ...
          <div class="content">
            <el-collapse v-model="rightMenuOpen">
              <el-collapse-item title="二级菜单" name="2">
                ...
                <el-button @click="showTwoMenu" style="width:100%;height:45px;" type="primary">添加</el-button>
              </el-collapse-item>
            </el-collapse>
          </div>
        </div>
...
import { addCategory, deleteCategory, searchCategoryList, filterCategoryList, findCategoryId, updateCategory } from '../sever'
...
export default {
  ...
  methods: {
    ...
    filterSearh(name, type) { // 根据条件查询一二级菜单
      ...
      filterCategoryList(param).then(res => {
        if (res.resCode == 200) {
          if (!type) {
    ...
    deleteMenu(obj) { // 删除菜单
      const that = this;
      deleteCategory({ ids: [obj.id] }).then(res => {
        if (res.resCode == 200) {
