  ...
  <div class="roleManage">
    <el-card class="box-card">
      ...
      <div class="table-operate">
        ...
        <el-button type="info" @click="batchTopicPublish(0)" plain>批量生效</el-button>
        <el-button type="info" @click="batchTopicPublish(1)" plain>批量无效</el-button>
      ...
      >
        ...
        <el-table-column prop="unitName" label="入驻单位名" min-width="130"></el-table-column>
        <!-- <el-table-column prop="defaultRole" label="是否默认角色" min-width="80">
          <template slot-scope="scope">{{ scope.row.defaultRole == 1 ? '是' : '否' }}</template>
        </el-table-column>-->
        ...
        <el-table-column prop="status" label="店铺默认角色" min-width="100">
          <template slot-scope="scope">
            <el-switch
              v-model="scope.row.defaultRole == 1 ? true : false"
              active-color="#13ce66"
              inactive-color="#ff4949"
              @change="bindClickDoDefault(scope.row.defaultRole, scope.row.id)"
            ></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="right" width="150px">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="showRoleDialog(scope.row)">权限 |</el-button>
    ...
    >
      <el-form :rules="rules" :model="form" ref="form">
        ...
        <el-form-item label="关联单位" prop="unitId" :label-width="formLabelWidth">
        ...
        <el-form-item label="默认角色" prop="defaultRole" :label-width="formLabelWidth">
    ...
    <el-dialog :title="role_name + '权限列表'" :visible.sync="dialogRoleVisible">
      <el-tree :data="roletree" show-checkbox ref="roletree" node-key="id" :props="defaultProps">
        <span class="custom-tree-node" slot-scope="{ node, data }">
          ...
          >{{ node.childNodes.filter(v => v.checked).length + '/' + node.childNodes.length }}</span>
        </span>
      </el-tree>
      <div slot="footer" class="dialog-footer">
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        defaultRole: [{ required: true, message: '请选择', trigger: ['blue', 'change', 'click'] }],
      },
    };
  },
  watch: {
  ...
  methods: {
    ...
    batchTopicPublish(status, val) {
      ...
      const url = status === 0 ? '/sys/storemanager/role/enable' : '/sys/storemanager/role/disable';
      if (val) {
        this.ids = [val];
      }
    ...
    // 设置默认角色
    bindClickDoDefault(status, val) {
      const url = status === 1 ? '/sys/storemanager/role/cancel-default' : '/sys/storemanager/role/set-default';
      this.$ajax
        .post(url, {
          id: val,
          ids: [val],
    ...
    delbtn(val) {
      ...
      if (this.ids.length === 0) {
        ...
        this.$confirm('确认删除？')
          .then(_ => {
            this.$ajax
              .post('/sys/storemanager/role/delete', {
                ids: this.ids,
              })
              .then(res => {
                if (res.resCode === 200) {
    ...
    showRoleDialog(val) {
      ...
      this.$ajax
        .get('/sys/storemanager/role/getRoleMenu', {
          params: {
            roleId: val.id,
          },
        })
    ...
    showEditDialog(val) {
      if (val) {
        this.$ajax
          .get('/sys/storemanager/role/roleInfo', {
            params: {
              id: val.id,
            },
          })
    ...
    // 提交
    submit(val) {
      const url = this.form.id ? '/sys/storemanager/role/edit' : '/sys/storemanager/role/add';
    ...
    // 提交权限
    submit_role(val) {
      const url = '/sys/storemanager/role/setRoleMenu';
      const data = {
        roleId: this.role_id,
