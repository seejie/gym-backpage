...
export default {
  ...
  methods: {
    ...
    getActiveList () {
      ...
      this.$ajax.get('/sys/THuigouTiyanProduct/findList', {
        params: {
    ...
    change (val) {
      console.log(val)
      this.$ajax.get('/sys/THuigouTiyanProduct/verifySingup', {
        params: {
          idsArr: val.id,
        ...
        if (res.resCode === 200) {
          this.$message.success(val.signStatus === 1 ? '审核成功' : '审核关闭成功')
        }
      }).catch((err) => {
        console.log(err)
      })
    ...
    getSinuplists () {
      ...
      this.$ajax.get('/sys/THuigouTiyanProduct/getProductUserPage', {
        params: {
          ...
          customerName: this.dialogFrom.selectType === '1' ? this.dialogFrom.input : '',
          customerMobile: this.dialogFrom.selectType === '2' ? this.dialogFrom.input : ''
        }
      }).then((res) => {
        if (res.resCode === 200) {
          this.moreDataShow = true
          this.sinuplists = res.resObject.list.map((res) => {
            return {
              headUrl: res.signup.headUrl,
              signStatus: res.signup.signStatus,
              customerMobile: res.order.customerMobile,
              customerName: res.order.customerName
            }
          })
          this.signTotalCount = Number(res.resObject.total)
        }
      }).catch((err) => {
        console.log(err)
      })
    ...
    // 弹框确定事件
    commit (val) {
      this.$ajax.get('/sys/THuigouTiyanProduct/updateDeletes', {
        params: {
    ...
    goLine (item) {
      if (item.publishStatus == 1) {
        this.$confirm('确认下线？')
          .then(_ => {
            this.$ajax.get('/sys/THuigouTiyanProduct/updatePublish', {
              params: {
    ...
    async getLabelData (id) {
      const arr = []
      this.$ajax.get('/sys/THuigouTiyanCategory/findAll', {
        params: {
          catGrade: 1
        }
      }).then((res) => {
