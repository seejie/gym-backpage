        ...
        <div class="item">
          ...
          <div class="content">
            <el-collapse v-model="leftMenuOpen">
              <el-collapse-item title="一级菜单"
                                name='1'>
                <div :class="currenIndex==index?'menu-item active':'menu-item'"
                     v-for="(item,index) in menulist"
                     @click="chooseItem(item,index)"
                     :key="index">{{item.catName}}<i class="el-icon-edit-outline icon-edit"
                     @click="edit(item)"></i> <i class="el-icon-delete icon-delete"
                     @click="del(item)"></i></div>
                <el-button @click="addMenu(1)"
        ...
        <div class="item">
          ...
          <div class="content">
            <el-collapse v-model="rightMenuOpen">
              <el-collapse-item title="二级菜单"
                                name='2'>
                <div v-for="(item,index) in nextMenuList"
                     :key="index"
                     @click="chooseNextItem(item,index)"
                     class="menu-item active">{{item.catName}}<i @click="edit(item)"
                     class="el-icon-edit-outline icon-edit"></i> <i @click="del(item)"
                     class="el-icon-delete icon-delete"></i></div>
                <el-button @click="addMenu(2)"
                           style="width:100%;height:45px;"
                           type="primary">添加</el-button>
              </el-collapse-item>
            </el-collapse>
    ...
    <el-dialog title="创建品类"
      ...
      <el-form :model="addFrom">
        ...
        <el-form-item label="品类图标"
                      label-width="100"
                      prop="catImg">
          <img-include v-model="addFrom.catImg"
                       :max="1"
                       :cropper="cropper"
                       :style="{ clear: 'both' }" />
        </el-form-item>
    ...
    <el-dialog title="编辑品类"
      ...
      <el-form :model="addFrom">
        ...
        <el-form-item label="品类图标"
                      label-width="100"
                      prop="catImg">
          <img-include v-model="editFrom.catImg"
                       :max="1"
                       :cropper="cropper"
                       :style="{ clear: 'both' }" />
        </el-form-item>
...
import * as http from '@/api/huigou/ticket/typeSet.js'
export default {
  data () {
    return {
      levelOneMenu: {
  ...
  methods: {
    ...
    edit (item) {
      ...
      this.editFrom.catParentId = item.catParentId
    },
    // 删除品类
    del (item) {
      console.log(item)
      const delId = item.catId
    ...
    delRow (item, id) {
      ...
      http.delType({
        ...
        item.catGrade === 1 ? this.queryAllType() : this.getNextMenuList()
      }).catch((err) => {
        console.log(err)
      })
    },
    ...
    getNextMenuList (id = this.currenParentId) {
      http.getType({
        ...
        this.nextMenuList = res.resObject
      }).catch((err) => {
        console.log(err)
      })
    },
    ...
    queryAllType () {
      http.getType({
        catGrade: 1,
        // catName: this.levelOneMenu.input ? this.levelOneMenu.input : null
      }).then((res) => {
        this.menulist = res.resObject
        try {
          this.currenIndex = 0
          if (res.resObject.length > 0) {
            this.currenParentId = res.resObject[0].catId
          } else {
            // 没有查到数据
            this.nextMenuList = []
          }
    ...
    confirmAdd () {
      if (this.addMenuType == 1) {
        ...
        http.addType({
          ...
          catGrade: 1,
          catImg: this.addFrom.catImg.ratio1 === undefined ? '' : this.addFrom.catImg.ratio1.join(','),
        }).then((res) => {
          // 重新获取一级菜单
          if (res.resCode == 200) {
    ...
    confirmEdit () {
      if (this.addMenuType == 1) {
        ...
        console.log(this.editFrom.catImg.ratio1)
        http.editType({
          catName: this.editFrom.name,
          catGrade: 1,
          catImg: this.editFrom.catImg.ratio1 === undefined ? '' : this.editFrom.catImg.ratio1.join(','),
          catId: this.currenId
        }).then((res) => {
          // 重新获取一级菜单
          if (res.resCode == 200) {
        ...
        http.editType({
          catName: this.editFrom.name,
          catGrade: 2,
          catParentId: this.editFrom.catParentId,
          catImg: this.editFrom.catImg.ratio1 === undefined ? '' : this.editFrom.catImg.ratio1.join(','),
          catId: this.currenId
        }).then((res) => {
          if (res.resCode == 200) {
