...
export default {
  data () {
    return {
      ...
      formItems: [
        ...
        { label: '码商筛选', model: 'coder', placeholder: '请输入码商', type: 'text' },
      ],
      dialogFrom: {
        input: '',
        selectType: '1'
      ...
      listColums: [
        ...
        {
          hasSort: false, // <Boolean> 是否排序
          prop: 'coder', // <String>  对应属性名
          label: '码商', // <String>   表头标签
        },
        {
          hasSort: false, // <Boolean> 是否排序
  ...
  methods: {
    ...
    // 弹框确定事件
    commit (val) {
      this.$ajax.get('/sys/THuigouTicketProduct/updateDeletes', {
        params: {
    ...
    changeComment (val) {
      console.log(val)
      this.$ajax.post('/sys/THuigouTicketProduct/update', {
        product: {
          id: val.id,
    ...
    goLine (item) {
      if (item.publishStatus == 1) {
        this.$confirm('确认下线？')
          .then(_ => {
            this.$ajax.get('/sys/THuigouTicketProduct/updatePublish', {
              params: {
    ...
    async getLabelData (id) {
      const arr = []
      this.$ajax.get('/sys/THuigouTicketCategory/findAll', {
        params: {
          catGrade: 1
        }
      }).then((res) => {
