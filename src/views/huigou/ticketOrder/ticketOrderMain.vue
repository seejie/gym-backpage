...
export default {
    ...
    return {
      ...
      btnList: [
        ...
        },
        {
          btnText: '批量退款',
          type: '',
          isShow: this.activeName !== '31' ? 'false' : 'true',
          cb: this.drawBack
        },
        {
          btnText: '导出',
  ...
  created () {
    ...
    } else if (this.activeName === '31') {
      this.getOrderList('', '', '31')
    } else if (this.activeName === '36') {
      this.getOrderList('', '', '36')
    } else {
  ...
  methods: {
    ...
    // 查询订单列表
    getOrderList (orderStatus, orderIscommit = 0, postSaleStatus = '') {
      this.$ajax.get('/sys/order/findPageList', {
        params: {
          ...
          orderIscommit: orderIscommit, // 待评价
    ...
    hexiao () {
      this.$ajax.put('/sys/order/confirmReceive', {
        orderCodes: this.orderId,
      }).then((res) => {
        console.log(res)
    ...
    // 退款
    backMoney () {
      this.$ajax.put('/sys/order/applyPostSale', {
        orderCodes: this.orderId,
      }).then((res) => {
        console.log(res)
        this.detailShow = false
