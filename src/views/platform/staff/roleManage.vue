    ...
    <div class="roleManage">
        <el-card class="box-card">
            ...
            <el-table ref="multipleTable" @selection-change="handleSelectionChange" :data="tableList" tooltip-effect="dark" style="width: 100%">
                ...
                <el-table-column prop="nameEn" label="英文名" min-width="160"> </el-table-column>
                ...
                <!-- <el-table-column prop="data_scope" label="数据范围" min-width="160">
                  <template slot-scope="scope">
                    {{ scope.row.data_scope == 1 ? "仅限本单位" : "本单位及下属单位 "}}
                  </template>
                </el-table-column> -->
        ...
        <el-dialog @close="handlerclose" :title="form.id ?'编辑角色' : '创建角色'" :visible.sync="dialogFormVisible">
          <el-form :rules="rules" :model="form" ref="form">
            ...
            <!-- <el-form-item label="数据范围" prop="data_scope" :label-width="formLabelWidth">
              <el-select v-model="form.data_scope" placeholder="请选择">
                <el-option label="仅限本单位" value="1"></el-option>
                <el-option label="本单位及下属单位" value="2"></el-option>
              </el-select>
            </el-form-item> -->
            <el-form-item label="关联部门" prop="unitId" :label-width="formLabelWidth">
              <el-select v-model="form.unitId" placeholder="请选择">
                <el-option :label="item.name" :value="item.unitId" v-for="item in units" :key="item.unitId"></el-option>
              </el-select>
            </el-form-item>
          </el-form>
        ...
        data() {
            return {
              ...
              rules: {
                ...
                nameEn: [
                  { required: true, message: '请英文名称', trigger: 'blur' }
                ],
                unitId: [
          ...
          append (val , vll) {
            ...
            delbtn(val) {
              if(this.ids.length==0 && !val){
                ...
                this.$confirm('确认删除？')
                .then(_ => {
                  this.$ajax.post('/sys/role/delete', {
            ...
            batchAble(){
              if(this.ids.length==0){
                ...
                this.$confirm('确认？')
                .then(_ => {
                  this.$ajax.post('/sys/role/batchAble', {
            ...
            batchDisable(){
              if(this.ids.length==0){
                ...
                this.$confirm('确认？')
                .then(_ => {
                  this.$ajax.post('/sys/role/batchDisable', {
                    ids: this.ids.map(v => v.id).join(',')
                  })
                    .then(res => {
                      if (res.resCode == 200) {
            ...
            // 权限
            showRoleDialog(val) {
              this.$ajax.get('/sys/role/getRoleMenu', {
                params: {
                }
              })
              .then(res => {
            ...
            showEditDialog(val) {
              if (val) {
                this.$ajax.get('/sys/role/roleInfo', {
                  params: {
                    id: val.id
                  }
                })
                .then(res => {
                  if (res.resCode == 200) {
                    this.form = {
                      ...
                      data_scope: res.resObject.data_scope + '',
            ...
            submit_role(val) {
              ...
              let data = {
                roleId: this.role_id,
                menuIds: this.$refs.roletree.getCheckedNodes().map( v => v.id)
              }
            ...
            // 列表
            getList() {
              this.$ajax.get('/sys/role/roleList', {
                params: {
