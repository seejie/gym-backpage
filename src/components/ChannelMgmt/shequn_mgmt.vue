...
<script>
  import {addShequPageSetting, modifyShequPageSetting} from "@/api/apiPlatformSetting"
  ...
  export default {
    ...
    methods: {
      getPageSetting() {
        this.$ajax
          .get("/shequ/pagesetting/getPageSetting", {})
          .then(res => {
            if (res.resObject) {
      ...
      loadChannelInfo() {
        this.$ajax
          .get(`/platformsetting/channel/getChannelByUnitIdAndChannelType?channelType=${channelData.shequ.channelType}`, {})
          .then(res => {
            if (res.resObject) {
