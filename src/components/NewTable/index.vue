<template>
  <el-card class="card">
    <div class="button-wrap" v-if="hasTopButton">
      <template v-for="(item,index) in topBtnList">
        <el-button
          :type="item.type?item.type:''"
          :key="index"
          v-if="item.isShow && item.isShow === 'false'?false:true"
          @click="handleTopBtn(item)"
        >{{item.btnText}}</el-button>
      </template>
    </div>
    <el-table :data="data" style="width: 100%" @selection-change="handleSelectionChange">
      <!--选择-->
      <el-table-column v-if="hasSelection && data.length!==0" type="selection" width="55"></el-table-column>
      <!--序号-->
      <el-table-column v-if="hasIndex && data.length!==0" type="index" width="55"></el-table-column>
      <!--数据源-->
      <template v-for="(column, index) in columns">
        ...
        >
          <template slot-scope="scope">
            <div class="title">
              <div class="img-wrap">
                <img :src="scope.row.title.imgUrl" alt />
              </div>
              <div class="text-wrap">
        ...
        <!-- 状态 -->
        <el-table-column :key="index" v-else-if="column.label==='发布状态'" :label="column.label">
          <template slot-scope="scope">
            <div v-html="getStatus(scope.row.publishStatus)"></div>
          </template>
        </el-table-column>
        <!-- 订单状态 -->
        <el-table-column :key="index" v-else-if="column.label==='订单状态'" :label="column.label">
          <template slot-scope="scope">
            <div v-html="getOrderStatus(scope.row.orderStatus)"></div>
          </template>
        </el-table-column>
        <!-- 审核状态 -->
        <el-table-column :key="index" v-else-if="column.label==='审核状态'" :label="column.label">
          <template slot-scope="scope">
            <div v-html="getReviewStatus(scope.row[column.prop])"></div>
          </template>
        </el-table-column>
        <!-- 关联状态 -->
        <el-table-column :key="index" v-else-if="column.label==='关联状态'" :label="column.label">
          <template slot-scope="scope">
            <div v-html="getLinkStatus(scope.row[column.prop])"></div>
          </template>
        </el-table-column>
        <!-- 排序 -->
        <el-table-column :key="index" v-else-if="column.type==='sort'" :label="column.label">
          <template slot-scope="scope">
            <el-input-number
        ...
        <!-- 图片 -->
        <el-table-column :key="index" v-else-if="column.type==='img'" :label="column.label">
          <template slot-scope="scope">
            <div class="avatar">
              <img :src="scope.row[column.prop]" alt />
            </div>
          </template>
        </el-table-column>
        <!-- 开关 -->
        <el-table-column :key="index" v-else-if="column.type==='switch'" :label="column.label">
          <template slot-scope="scope">
            <div class="switch">
              <!--  -->
              <el-switch
        ...
        <el-table-column :key="index" v-else-if="column.type==='switchs'" :label="column.label">
          <template slot-scope="scope">
            <div class="switch">
              <el-switch
                v-model="scope.row[column.prop]"
          ...
          <template slot-scope="scope">
            <div class="operat">
              <el-link
                ...
                @click="handleOperation(scope.row,item)"
              >{{item.btn}}</el-link>
              <el-dropdown v-if="column.isMoreShow">
                <el-link type="success" :underline="false" style="color:#409EFF;">
                  更多
      ...
      <!--体验活动页面单独的操作列表-->
      <el-table-column label="操作" v-if="operate.length!==0" width="150">
        <template slot-scope="scope">
          <el-link
            type="success"
            :underline="false"
          ...
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              >{{scope.row.publishStatus ==1?'下线':'上线'}}</el-dropdown-item>
              <!-- <el-dropdown-item @click.native="forbid(scope.row)">{{scope.row.isComment==0?'禁言':'取消禁言'}}</el-dropdown-item> -->
              <el-dropdown-item @click.native="goTop(scope.row)">{{scope.row.stick==0?'置顶':'取消置顶'}}</el-dropdown-item>
              <el-dropdown-item @click.native="del(scope.row)">删除</el-dropdown-item>
              <el-dropdown-item @click.native="comment(scope.row)">评论</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
...
export default {
  ...
  computed: {
    getStatus() {
      return function(val) {
        if (val == '1') {
          return "<span style='color:#29bc4f'>已上线</span>";
        } else if (val == '0') {
    ...
    getOrderStatus() {
      return function(val) {
        console.log(val);
        if (val == 0) {
          return "<span style='color:#909399'>待付款</span>";
        } else if (val == 1) {
          return "<span style='color:#67C23A'>待核销</span>";
        } else if (val == 3) {
          return "<span style='color:#67C23A'>已核销</span>";
        } else if (val == 4) {
          return "<span style='color:#67C23A'>已取消</span>";
        }
      };
    },
    //审核状态
    ...
    isDisabled(row, val) {
      return function(row, val) {
        if (row.btn == '核销') {
          ...
          if (val.postSaleStatus === 31) {
            return false;
          } else {
            return true;
          }
        } else if (row.btn == '审核驳回') {
